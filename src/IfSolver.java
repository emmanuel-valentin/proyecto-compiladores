...
public class IfSolver {
  ...
  private void solve(Nodo n) {
    ...
    Nodo parentIfBody = new Nodo(new Token(TokenType.SEMICOLON, ";"));
    Nodo parentElseBody = new Nodo(new Token(TokenType.SEMICOLON, ";"));

    if (children.get(children.size() - 1).getValue().getType() == TokenType.ELSE) {
