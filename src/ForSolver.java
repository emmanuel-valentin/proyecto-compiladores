...
public class ForSolver {
  ...
  private void solve(Nodo n) {
    ...
    Nodo parentBody = new Nodo(new Token(TokenType.SEMICOLON, ";"));
    ...
    // Es una declaración
    if (assignDeclaration.getValue().getType() == TokenType.VAR) {
      VarSolver varSolver = new VarSolver(assignDeclaration);
      varSolver.solve();
    }
    // Es una asignación, la variable ya ha sido declarada
    else if (assignDeclaration.getValue().getType() == TokenType.ASSIGN) {
